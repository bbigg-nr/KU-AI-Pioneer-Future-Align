...
with open("test_results.json", "w", encoding="utf-8") as out:
    results = []
    ...
    try:
        payload = {"messages": [{"role": "user", "content": "Find jobs that match my skills"}], "student_id": "STU_0001"}
    ...
    try:
        payload = {"messages": [{"role": "user", "content": "Analyze my skill gap for Software Engineer"}], "student_id": "STU_0001"}
        r = requests.post(f"{BASE}/ai/advisor/chat", json=payload, timeout=60)
        d = r.json()
    ...
    try:
        payload = {"messages": [{"role": "user", "content": "Recommend courses to improve Python and SQL"}], "student_id": "STU_0001"}
        r = requests.post(f"{BASE}/ai/advisor/chat", json=payload, timeout=60)
        d = r.json()
    ...
    print(f"Wrote {len(results)} test results to test_results.json")
